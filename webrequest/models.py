...
class TelegramReportRequest(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    document1 = models.FileField(upload_to = 'requests/')
    document2 = models.FileField(upload_to = 'requests/')
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.id:
            self.created = timezone.now()
        self.modified = timezone.now()
        return super(TelegramReportRequest, self).save(*args, **kwargs)

class TelegramReportResponse(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    request = models.ForeignKey(TelegramReportRequest, blank=True, null=True, on_delete=models.CASCADE)
    report = models.FileField(upload_to = 'responses/')
    created = models.DateTimeField(default=timezone.now(), editable=False)
    modified = models.DateTimeField(default=timezone.now())
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.id:
            self.created = timezone.now()
        self.modified = timezone.now()
        return super(TelegramReportResponse, self).save(*args, **kwargs)
